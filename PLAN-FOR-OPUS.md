...
>
> **⚡ REFRAME (19 Jul, Shazin insight): P1.4/1.5 is NOT a prerequisite for onboarding — because we're KILLING auto-gen.** P1.4/1.5 was "merge the two auto-content pipelines (scheduler + intl-seo) into one module" = house-cleaning of code we're now TURNING OFF. So a new brand does NOT need that cleanup to be useful. What makes Southpour/Yolk genuinely useful, day one, is: (1) config (P2), (2) research (keyword-discovery → worklist, vertical-adapted), (3) the DATA jobs (GSC snapshots/rank tracker/traffic — already per-brand), (4) the **on-demand ⚡Generate module** (the ONE content path now), (5) publish (WP per brand). None of that needs the legacy-pipeline extraction.
> **So P1.4/1.5 transforms:** from "carefully extract + merge two crufty auto-gen pipelines" → to "build ONE good on-demand generation lib that carries the GOOD intelligence (page-architecture guardrails [done], competitor context, cannibalization, brand voice, and — for café/local — SERP/GBP routing), config-driven for any brand; then just SWITCH OFF (later delete) the legacy auto-gen orchestration." That's user-facing value (a Generate button that works well for any brand), not house-cleaning, and it's non-blocking cleanup afterward.
> **NET near-term critical path (all user-facing, all this week-able):** brandsConfig (P2) → on-demand Generate module w/ intelligence (the real content path) → onboard Southpour + Yolk → they research + generate + publish in their own voice/vertical. The old auto-gen code gets flipped off (1-line) and deleted at leisure. P1.2 voice-gate / P1.3 GSC-path fold into the config work (same ternary). Nothing valuable is blocked on "cleaning house."
