...
- **7b — Outcomes view:** build the lens on `seoEvents` + `pageSnapshot` + registry; retire `content-outcomes`' keyword-matching in favour of page-attribution. Replaces Published & Tracking (F).
  - ✅ **DONE (v7.9.81, live-verified 2026-09-14).** New Analytics → **Outcomes** sub-tab (first tab, alongside the rest). Per brand: tiles (shipped · published/edited · now-ranking · clicks driven), "what's working" by page type, table of every shipped page → current pos/clicks/status. Live proof: Bonbird shows venue/city_hub 0/3 ranking vs page/home ranking — surfacing "venue work hasn't paid off yet" automatically; noindex flagged. Movement-since-ship deltas fill in as snapshots accrue. `content-outcomes` retirement folds into 7c.
