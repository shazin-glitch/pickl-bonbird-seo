...
- **7a — SEO event log** (`seoEvents:<brand>`): start recording work events now (publish/edit/task-done) + backfill from the registry. Pure addition; nothing changes visibly yet. *This is the keystone — history only accrues once it's running.*
  - ✅ **DONE (v7.9.80, live-verified 2026-09-14).** Captured from sitemap `<lastmod>` (free): new content page → `published`, advanced lastmod → `edited` — covers manual WP edits AND Nest publishes. Backfills nestCreated pages on first run. `seoEvents:<brand>` (append-only, cap 1000); read via `/api/page-registry?events=1`. Bonbird backfilled 12 events, Pickl 5. Caveat: legacy pages without a sitemap lastmod get a best-effort date (today); real content pages carry accurate dates.
- **7b — Outcomes view:** build the lens on `seoEvents` + `pageSnapshot` + registry; retire `content-outcomes`' keyword-matching in favour of page-attribution. Replaces Published & Tracking (F).
  - ✅ **DONE (v7.9.81, live-verified 2026-09-14).** New Analytics → **Outcomes** sub-tab (first tab, alongside the rest). Per brand: tiles, "what's working" by page type, table of every shipped page → current pos/clicks/status. `content-outcomes` retirement folds into 7c.
  - ✅ **POLISH + SCALE (v7.9.82, live-verified 2026-09-14).** Shazin flagged the first cut had bad taste + wouldn't scale. Rebuilt to **summary → attention → detail**: window selector (30/90d/all), KPI tiles, a **segment rollup** (toggle by market / page type; stacked ranking/building/fix bar per segment; **click to drill** — expands inline + filters the table), attention strip, and a **searchable / status-filterable / sortable / paginated** table with a position-Δ column (from the last two weekly snapshots). Verified live: Oman 1/7, UAE 3/3, Qatar 0/2; clicking Oman drills to its 7 pages. This is the visual language + IA to reuse for State of SEO + Trends in **7c**. (Direction chosen via 2 artifact mocks; the "at-scale" interactive mock won.)
- **7c — Consolidation:** reorganize Analytics into the 4 lenses; retire the Rankings triple-stack (A/C) and the duplicate top-10 sources (G/H); fold Markets (I) into State. **Meticulous rule: migrate each valued piece (traffic value, position bands, movers, AI Search) into its new home and verify it renders before deleting the old surface.**
  - 🟡 **IN PROGRESS — ADDITIVE PART DONE (v7.9.84–86, live-verified 2026-09-15). RETIREMENT DEFERRED by Shazin ("only retire when we're sure everything we need is covered").** Built: (1) monthly per-market data layer `monthlyTrend:<brand>` backfilled from GSC (~13 months); (2) **Trends** tab — MoM impressions-first, toggle-compare multi-line chart (scales to any market count, auto palette), 6/12-mo window, per-market MoM table, in-progress month dashed/marked, KPIs off the last complete month; (3) **State of SEO** gained an impressions tile + friendly market names (rollup already had impressions). Rankings + Report tabs LEFT IN PLACE. ~~STILL TO DO before retiring: bring position-distribution + rank-movers + traffic-value(AED) + AI-Search into Trends~~ ✅ **DONE (v7.9.87/87b, live-verified 2026-09-15):** all four migrated into Trends below the monthly chart — position distribution (non-branded bands), traffic value (AED) + non-branded clicks, AI presence (% keywords w/ AI Overview via `hasAiOverview`, + LLM mentions as `results[].mentioned`/total), and rank movers (rank-tracker top-movers/needs-attention). **Coverage for retirement is now complete.** REMAINING (awaiting Shazin's OK): confirm coverage, THEN retire the Rankings triple-stack (A/C) + dedupe the top-10 counts (G/H) + fold Markets (I) into State + retire content-outcomes keyword-matching / deferred query-only path. Retire nothing until Shazin confirms.
- **7d — Analysis layer:** CTR gap + win/loss-by-segment + time-to-rank, with CTR fixes auto-flowing to Perch.
  - ✅ **CTR-gap DONE (v7.9.88, live-verified 2026-09-15):** `perch-sync` flags pages ranking page-one (pos ≤10, impr ≥200, clicks ≥1) with CTR < 2% → auto Perch task "Low CTR — <url> (#pos, rate%)". 5 real tasks (/dubai/ #8.1 @0.6%, /ae/dubai/ #7.3 @0.7%, homepage #9.4 @1.9%, …). Auto-flows to Perch as designed.
  - ✅ **Win/loss by segment ALREADY DONE** — Outcomes "What's working" has a By market / By page type toggle with ranking/building/fix bars per segment (v7.9.82).
  - ⏳ **Time-to-rank DEFERRED (data):** needs several weekly `pageSnapshot`s to measure the week a page actually crossed to page 1 — only 1–2 exist now, so a real number isn't possible yet (a proxy would mislead). Wire it once ~4–6 weeks accrue.
