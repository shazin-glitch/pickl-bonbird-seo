...
- **P1.5 — config-driven assets merged (v7.9.18).** `buildMarketPlan` now merges **city hubs from `citiesForMarketAsync`** (venue config, research-independent) into the plan, deduped vs queued. So a market DataForSEO doesn't cover still gets a real plan. Learned live: **Oman & Qatar are NOT in DataForSEO Labs** (`ideasDiag: "… not in DataForSEO Labs"`, ideasFetched:0) → keyword-research plans there are thin, but city-hub + product + seed-journal assets (config/menu-driven) fill the gap. Pakistan (362 ideas) + UAE (200) are research-rich. Data confirmed fresh (Aug 24 run). (P2b ✅ DONE v7.9.34 — `preflightPageCreations` matches plan items to the 12 real product scaffolds and fills them; non-matching product intent creates on `template-product.php`; venue-page keywords are blocked until their city hub exists.)
...
- **P3 — EXECUTE (two halves):**
  ...
  - **P3a ✅ DONE (v7.9.25)** — `action:'execute'` (dryRun DEFAULT true, manager/admin only, plan must be ready, no-selection = nothing runs, cap 25) + `market-planner-execute-background.js` (sequential loop → `marketPlanRun:<brand>:<market>`, outcome per item: queued/skipped/routed/error) + `action:'run'` polling. Single shared mapping `planItemToDraftCall` so the dry run shows EXACTLY what the real run sends. Offline-verified with 50 assertions (mocked Blobs/fetch): dry run makes **0 network calls, 0 writes**. Live dry-run → ONE real item → batch still owed.
- **P2 VERIFIED LIVE (v7.9.23):** Bonbird Pakistan plan = **12 focused items** (mode llm) from 100+ keywords — 6 page_creation, 3 meta_update, 2 blog_draft, 1 city_hub; clustered (e.g. "nearest chicken shop to me +8"), relevant (no off-brand), meta targets real pages. ⚠️ LLM included "best burger in karachi" — Bonbird has NO Karachi venue. **FIXED v7.9.28** (venue-city prompt + hard guard), along with near-me→GBP routing, stale-target filtering and vertical-aware hints: the same plan now yields 5 clean items, 7 dropped with reasons.
...
## Status
- P1: ✅ DONE (v7.9.17) · P1.5 config-asset merge ✅ (v7.9.18) — `_lib/market-planner.js buildMarketPlan` read-only, mock-verified (cold + warm, dedupe, skip-winning, ranked, data-driven counts).  P2: ✅ DONE + VERIFIED LIVE (v7.9.18 backend · v7.9.19 panel · v7.9.20 assess-map · v7.9.21 Claude brain · v7.9.22-23 async background+poll) — 🗺️ Market Planner card (Analytics→Markets): brand+market → Build plan → ranked read-only map. Next: P3 execute.  P3: 🟡 **P3a ✅ DONE + LIVE-VERIFIED** (v7.9.25 execute backend · v7.9.26 relative-URL fix · v7.9.27 stale-target pre-flight · v7.9.28 plan-quality guards). Live: dry-run → pre-flight (12→10) → ONE real item → Approvals `itm_mt9bt4x0_ij567z`. Batch not yet run live. · **P3b dashboard ✅ DONE (v7.9.30-31)** — top-level **Content Planner** nav item (Create group, between Calendar and Approvals = the spine order). ONE brand+market control; the 🗺️/🏙️/📄 cards are DELETED from Analytics→Markets and folded in. Sections: `1 · Plan` (checkbox list + guard-skipped reasons) → `2 · Generate` (dry-run preview → generate → progress → Approvals link) → `Also ready to generate`. `mode:'rules'` surfaced as a visible fallback warning. **Batch generate is still un-run live** (only the single smoke-test item has been fired).  P4: ⬜ (deferred).
...
**✅ BUILT v7.9.30 as specified.** Original direction: **Design P3b around the SEO workflow, one coherent spine:** Discover → **Plan** → Generate → Review → Publish → Measure. The Market Planner should be the PRIMARY "act on a market" surface (its own destination, not a buried card), with Generate happening in-place on the plan, flowing into Approvals. FOLD the separate city-hubs + scaffold panels into the planner (or retire them) rather than adding a parallel one. Decide placement (own nav item vs top of Markets) with Shazin before building P3b.
...
- **P3 refinement ✅ DONE (v7.9.28)** — `assetHint` per vertical in config + vertical noun in the system prompt; corporate explicitly forbids local/product framing. Original note: make `planWithClaude`'s asset-type guidance **vertical-aware** — don't frame a corporate brand around "local landing / product / city hub." Pass the vertical's `promptNoun` + a per-vertical asset hint into the prompt. Small addition; do it in P3 so Yolk/Southpour plans read correctly.
