...
## Current Version: v7.4.21

Last built (v7.4.21): KSA matrix all-blank fix — KD/volume + competitor ranks + aggregator leak. Reported as "KSA refresh shows nothing — Vol/mo, KD, competitor data all empty." Three real root causes, all live-verified against the DataForSEO API (creds pulled via `netlify env:get`):
