...
### v7.9.73 — Refinement plan P0.1 + P1: index-inspection fix + the live-page registry
First execution slice of `NEST-REFINEMENT-PLAN.md` (whole-tool reporting/coherence overhaul; audits + plan authored 2026-09-14). Two changes, both verified read-only first, additive, nothing retired.
- **P0.1 — URL-inspection siteUrl fix** (`scheduler-background.js:388`). It built the inspection `siteUrl` from the domain (`https://bonbirdchicken.com/`), but Bonbird's verified GSC property is `sc-domain:bonbirdchicken.com`, so every Bonbird `index:inspect` call 403'd and `indexStatus` never populated (Pickl unaffected — its property IS the URL-prefix form). Now uses the brand's canonical `gscProperty` (`BRANDS[brand].gsc` → `getBrand().gscProperty` → domain fallback); works for both `sc-domain:` and `https://` properties. Payoff shows once P1 gives the tracker live pages to inspect.
- **P1 — live-page registry** (NEW `page-registry-background.js` + `page-registry.js` + `/api/page-registry` redirect + wired into `cron-weekly-background`). The reporting inversion: the approval queue knew ~8 published pages while ~198 are live. The registry is the new source of truth for "what's live", built from FREE sources only — Yoast sitemaps (`sitemap_index.xml` → sub-sitemaps) ∪ GSC page list (`fetchGscPageOnly`, 90d) ∪ Nest approval URLs — so NO paid crawl (that's `onpage-audit-background`, monthly). Writes ONE new key `pageRegistry:<brand>` (does not touch `pageInventory:<brand>`). Per page: market (`getMarketPageTokens` attribution), pageType, inSitemap, indexable/robots (a stray `noindex` like `/om/chicken-tenders/` surfaces automatically — bounded live robots-fetch for pages missing from the sitemap), impressions/clicks/position, `nestCreated` (approval-URL join, best-effort), firstSeen/lastSeen (preserved across weekly runs). Config-driven (brands/markets from accessors). Gated: background = `authorizeJob`, read = `authorize`. Fired weekly by the dispatcher (free, safe weekly). `npm run check` green.

### v7.9.107 — Overhaul step G (part 1): retire the Markets tab
Removed the **Markets** analytics tab button — its reporting data (per-market impressions/clicks/rankings) is now covered by Performance (per-market MoM + custom date range) and State of SEO (by-market rollup); its only unique bits (content-queue-per-market, run log) are content-ops that belong to Content Planner, not reporting. Retired the SAFE way: removed the tab button only; the panel HTML + `loadIntlDashboard` stay dormant in the file (nothing destroyed, trivially restorable). Remaining G items: Report tab (retire once Performance has a PDF/export) and the Rankings tab (market-traffic is dup'd in Performance; its GSC top-query table needs a home first). `npm run check` green.

### v7.9.106 — Overhaul step E: unified Keywords lens (Rank Tracker + Opportunities)
Per architecture: one home for "what are we tracking / what to target next." Added a **Keywords** analytics tab that hosts both the Rank Tracker and Keyword Opportunities. Implemented as a runtime DOM RELOCATION (`keywordsLensRelocate`, idempotent, called from `switchAnalyticsTab`) — moves the existing `#rt-card` and `#panel-opportunities` nodes into `#kw-tracker-mount`/`#kw-opps-mount`, so their IDs + handlers + render functions keep working unchanged (no risky rebuild). Removed the standalone **Opportunities** tab (now inside Keywords) and dropped the Rank-Tracker loader from the Rankings tab (it lives in Keywords now; Rankings temporarily keeps GSC table + market-traffic pending step G retirement). `npm run check` green.

### v7.9.105 — Overhaul step D: State of SEO gets Site-health + Authority summaries
Per NEST-REPORTING-ARCHITECTURE §7 (Technical SEO + Backlinks stay their own tabs; State of SEO carries *summaries* that link out). `loadStateOfSeo` now also fetches `/api/backlinks` + `/api/technical-seo` (best-effort), and `renderStateOfSeo` shows two cards under the tiles: **Site health** (avg mobile/desktop PageSpeed + Core Web Vitals status + any technical issues → click opens the Technical SEO section via `switchView('techseo')`) and **Authority (backlinks)** (referring domains + Δ + domain score + dofollow% → click opens the Backlinks analytics tab). Completes D's additive scope; State of SEO now answers "what's live + how healthy + how authoritative" in one view. `npm run check` green.

### v7.9.104 — closed-loop for edits: measure if meta/content changes moved CTR (auto, over time)
Extends `content-outcomes-background` (the weekly closed-loop job) with `measureMetaOutcomes`: reads the `optimized` events from `seoEvents:<brand>` (the v7.9.103 work log), finds the weekly `pageSnapshot` taken just BEFORE each change (a true baseline, by builtAt ≤ change time) vs the latest snapshot, and — since snapshots carry clicks+impressions+position — computes the CTR/position/clicks delta. Reports a verdict once a change is ≥14 days old (improved/declined/flat; younger = "maturing"); meta edits judged on CTR (±0.2pp), content/publish on position/clicks. Fires ONE Slack result per change when it first grades (reused `seo_work` type, `action:'result'`, "CTR 0.8% → 2.1% ↑"; flat = "no change — revisit"), deduped via a `reportedOutcomeAt` stamp on the event. Results fold into `contentOutcomes:<brand>` (`metaResults`/`metaTotals`) and render in State of SEO as an "Optimization results — did our edits move CTR?" table + a "N still being measured" note. Runs on the existing Mon 6am cron, so 2–4-week results appear automatically. `npm run check` green.

### v7.9.103 — Nest announces + tracks every live change (Slack + work log)
Shazin: "when we publish something or make any edits that go live, send a Slack notification — good to see progress" + "ensure anything we implement is tracked so we can monitor over time." Built a reusable record+notify primitive: `wordpress.js` now calls `recordAndNotify()` on every successful live write — `update_meta` (captures before→after via a pre-read), `update_content`, and `publish`. It (1) appends an `{type:'optimized', action, url, detail:{before,after}}` event to `seoEvents:<brand>` (the work log the Outcomes view reads, so changes are tracked + monitorable against the weekly per-page snapshots over time) and (2) fires a new `seo_work` Slack type (`slack-notify.js`) — a CEO-facing "🪺 The Nest — live SEO work" message listing each change with before→after. Both best-effort (never fail the write). Meta writes are title/description only (never the body — the /ae/ homepage-clobber guard stands). `npm run check` green.

### v7.9.102 — registry: FOLD 301'd impressions into the redirect target (keep totals accurate)
v7.9.101 excluded the 71 detected redirects but that made UAE impressions collapse ~214K→48K, because the 301'd URLs (homepage 60K, /dubai/ 28K …) carry real demand GSC still attributes to the old URL during migration. Fix: instead of dropping their traffic, FOLD each redirect's impressions/clicks into its redirect TARGET (via `redirectTo`), then re-sort. Result: no duplicate rows AND accurate market totals attributed to the live `/ae/…` page (e.g. `/ae/dubai/` now shows the combined ~45K it's consolidating). Adds `foldedFromRedirect` on targets for transparency. `npm run check` green.

### v7.9.101 — registry: detect 301'd legacy URLs (stop showing redirects as live pages / double-counting)
Found while hunting UAE opportunities: the registry listed 301'd legacy URLs (`/dubai/`, `/uae-menu/`, `/abu-dhabi/`, `/locations/`, `/` …) as LIVE pages, double-counting their impressions in the UAE totals and producing a FALSE "cannibalization" reading. Root cause: the registry only HTTP-checked out-of-sitemap URLs that ALSO had zero impressions, so trafficked legacy URLs (which GSC keeps reporting for months after a 301 — normal migration lag) were never verified and defaulted to `indexNote:'has-impressions'` = live. Verified live that the 301s are actually correct (`/dubai/`→`/ae/dubai/` 301, etc.) — the redirects were never the problem; the registry's reporting was. Fix: HTTP-verify EVERY out-of-sitemap URL (prioritised by impressions, parallel via mapLimit, cap 150), check `redirect` BEFORE the has-impressions fallback, capture `redirectTo`, and EXCLUDE redirect pages from the live registry (`pages`) so they can't be shown as duplicates or inflate market totals. Added `summary.redirectsExcluded`. `robotsIndexable` now returns `finalUrl`. Replaced the old serial ROBOTS_FETCH_CAP loop. `npm run check` green.

### v7.9.100 — fix State-of-SEO crash (TDZ) + all 3 index-report pieces LIVE-VERIFIED
The v7.9.99 `concernsBlock` in `renderStateOfSeo` referenced the `th`/`td` consts before their declaration → `ReferenceError: Cannot access 'th' before initialization` (temporal dead zone), so State of SEO hung on "Loading live-page registry…". Moved concernsBlock below the th/td declarations. LIVE-VERIFIED all three pieces on Bonbird: (1) registry `summary.indexingConcerns = 7` (age 17–18d, the OM/QA hub+venue pages); (2) perch-sync auto-created 7 "Not indexed / not serving — <url> (Nd)" tasks (34 tasks total on the board); (3) State of SEO shows the "not indexed / stuck: 7" tile + the ⚠ Indexing concerns (7) drill-down (page · market · age · Google last-crawl). `npm run check` green.

### v7.9.99 — Nest now REPORTS not-indexed/stuck pages: durable signal + Perch alert + State-of-SEO line
Builds on v7.9.97/98 to actually surface index problems (Shazin: "I need the Nest to report if something isn't indexed"). Three coherent pieces, each in the surface that already owns the job: (1) DETECTION in `page-registry-background` — a durable "indexing concern" in the registry summary: a page that is indexable + in the sitemap + 0 impressions after being live STUCK_INDEX_DAYS (14) is flagged (`summary.indexingConcerns` + `indexingConcernUrls[]`). Aged from the EARLIER of sitemap lastmod and firstSeen (firstSeen alone undercounts pages live before the registry existed). This is trusted OVER the URL-Inspection verdict, which is Google's LAST-KNOWN state and can lag the live GSC UI (it said "Submitted and indexed" for pages the live check showed as not indexed) — the verdict now rides along only as `lastCrawlVerdict` detail. (2) ALERT in `perch-sync-background` — a new "Not indexed / not serving" finding → Perch task + Slack, deduped by sourceId, with URL + age + last-crawl detail. (3) VIEW in State of SEO (`renderStateOfSeo`) — a "not indexed / stuck" tile + a red drill-down table (page · market · age · Google last-crawl). `npm run check` green.

### v7.9.98 — index check: only URL-inspect 0-impression pages + LIVE-VERIFIED (corrects a site: misread)
v7.9.97 first-run inspected ALL ~200 pages → too slow (URL Inspection is ~5-15s/call), the job never finished writing. Fix: a page with impressions>0 is BY DEFINITION indexed (it appeared in results) → mark it indexed with no API call; only URL-Inspect indexable, non-redirect, 0-impression pages (~7 for Bonbird). Job now completes in seconds. LIVE-VERIFIED and it CORRECTED an earlier wrong conclusion: I had used a Google `site:` check and concluded the OM/QA hub/venue pages weren't indexed — but `site:` on deep URLs is unreliable. The authoritative URL Inspection API says 6 of those 7 are **"Submitted and indexed"** (`/om/muscat/`, `/om/muscat/souq-al-madina/`, `/om/seeb/`, `/om/seeb/al-khoudh/`, `/qa/doha/`, `/om/journal/bonbird-oman-opening/`) — they're indexed but get 0 impressions (a DEMAND/ranking gap, not an indexing gap). Only **`/qa/doha/district-one/`** is genuinely "Crawled - currently not indexed". Registry summary now: indexed 210, notIndexed 1. Lesson: `site:` is not ground truth; URL Inspection is. `npm run check` green; verified live (builtAt 14:18, indexChecked 211).

### v7.9.97 — Registry now reports REAL Google index status (URL Inspection), not just "indexable"
The registry conflated "indexable" (page ALLOWS indexing — no noindex) with "indexed" (Google actually has it), and used impressions as a proxy. So pages that were in the sitemap for weeks but never indexed (e.g. the Oman/Qatar city-hub + venue pages: `/om/muscat/`, `/qa/doha/`, `/qa/doha/district-one/` — confirmed not indexed via live `site:` check) looked fine. Fix: `page-registry-background` now calls the GSC **URL Inspection API** for every live indexable page and stores `indexState {verdict, coverageState, lastCrawlTime, indexed, checkedAt}` (indexed = verdict PASS). Quota-savvy: reuses a prior PASS when `lastmod` is unchanged, so it only (re)inspects new/edited/not-yet-indexed pages (cap 400/brand/run, concurrency 5; GSC quota 2000/day, 600/min). Skips noindex/redirect pages. Summary now carries `indexed`, `notIndexed`, `indexChecked`, and `notIndexedUrls[]`. This is the data layer for "the Nest tells me when something isn't indexed"; next: Perch alert + State-of-SEO surface. `npm run check` green.

### v7.9.96 — Perch "Not in sitemap": stop flagging redirects (false-positive fix)
Monday's cron created 4 "Not in sitemap" Perch tasks for Bonbird: `/`, `/dubai/`, `/sharjah/aljada/`, `/oman/`. All false positives — each is a **301 redirect** to its canonical ISO-market URL (`/`→`/ae/`, `/dubai/`→`/ae/dubai/`, `/oman/`→`/om/`), and the targets ARE in the sitemap. A redirecting URL correctly does NOT belong in a sitemap, so flagging it is wrong. Root cause: the registry pulls Nest-approval URLs (in their pre-migration non-prefixed form) into the universe, and `robotsIndexable()` used `fetchText` with `redirect:'follow'` — so it silently fetched the redirect TARGET's HTML, saw no noindex, and reported the URL "indexable" (the 301 was invisible). Fixes: (1) `page-registry-background.js robotsIndexable()` now does its own `fetch(redirect:'follow')` and returns `status:'redirect'` when `r.redirected` or a 3xx — so redirects are classified, not mistaken for live pages; (2) the registry status gains a `'redirect'` value (before the noindex check) so they're not mislabeled; (3) `perch-sync-background.js` "not in sitemap" filter now requires `indexNote === 'index'` (a CONFIRMED live 200 + indexable page), which excludes redirects, unreachable, and unknown in one go. perch-sync is add-only (dedups by sourceId, never removes), so the 4 already-created tasks must be dismissed by hand once — they won't recur. `npm run check` green.

### v7.9.95 — Speed & Vitals audit: re-pass errored pages once (mop up transient lighthouseError)
PSI's "lighthouseError / Something went wrong" is transient and moves page-to-page run-to-run (a page that scored fine last run flakes this run), so after v7.9.94 the core-page timeouts were gone but ~1-2 intl pages still showed a stray lighthouseError. Added PART 2b: after the main core+intl passes, collect the pages that errored and re-run PSI for just those once, in parallel (mapLimit). By now the whole rest of the audit has elapsed, so a fresh attempt almost always succeeds; on success the error entry is replaced in place with real scores (+ issues created), on repeat failure the recorded error is kept. Cheap (only the failures) and directly cuts the residual "errors here and there". `npm run check` green; verified live.

### v7.9.94 — Speed & Vitals audit: parallelise (mapLimit) + retry timeouts once (kill the intermittent aborts)
After v7.9.93, heavy pages (/ae/ home, /ae/locations/) STILL timed out intermittently — their PSI response time varies run to run, sometimes over the 90s window — and v7.9.93 deliberately did not retry timeouts (to protect the 15-min budget when the audit ran serially). Fix: (1) run pages CONCURRENTLY via a new `mapLimit(items, limit, fn)` helper (PSI_CONCURRENCY=4 pages in flight, ×2 strategies = ≤8 PSI calls, safe under quota) for BOTH the core and international loops — this cuts total wall-clock ~4x; (2) now that the budget is freed, retry a timeout ONCE (2 attempts, 3s pause) so a variable slow page gets a second chance, while fast failures (lighthouseError/5xx) still retry up to 2x. Worst-case per page bounded (2×90s timeout, or 3 fast tries) but pages overlap, so the whole audit still finishes well inside 15 min. Kept category=performance (v7.9.92) + 90s window (v7.9.93). Brand-agnostic. `npm run check` green; verified on a clean deployed audit.

### v7.9.93 — Speed & Vitals audit: 90s timeout + fast-failure-only retry (live-test correction to v7.9.92)
Live-tested v7.9.92 on the deployed function (authenticated Bonbird audit) and found `category=performance` alone was NOT enough: heavy pages still failed, and 2 of 3 were 45s ABORT TIMEOUTS, not lighthouseError — i.e. PSI performance-only genuinely takes longer than 45s to return for these pages from Netlify's environment, so the old `AbortSignal.timeout(45000)` was killing them mid-analysis. Also, retrying timeouts (as v7.9.92 did) multiplied wall-clock (~140s/page) and risked blowing the 15-min background-function budget before the audit finished. Fix: (1) raise the per-call timeout to 90s (affordable — pages run serially inside a background fn); (2) retry ONLY fast failures (5xx/lighthouseError, which return quickly) up to 2x with a 5s/10s pause; do NOT retry a genuine timeout (it already had the full 90s; re-spending 90s×every slow page is futile and budget-threatening — the larger window IS the timeout mitigation). Kept category=performance from v7.9.92. `npm run check` green; verified on a clean deployed audit run.

### v7.9.92 — Speed & Vitals audit reliability: PSI performance-only + retry (fixes lighthouseError 500s)
The Speed & Vitals audit (`technical-seo-background.js` `runPageSpeed`) was failing on heavy pages (Bonbird UAE home/menu/locations/franchise/philosophy/dubai) with `PSI 500 lighthouseError: Something went wrong` + timeouts for 3+ hours, while light pages passed. Root cause: the PSI request had NO `&category` filter, so PSI ran the FULL Lighthouse (performance + accessibility + best-practices + SEO + PWA) — far heavier — and heavy pages blew past the 45s window or tripped PSI's transient lighthouseError, with no retry. Fix: (1) request `&category=performance` ONLY (verified no consumer of the other categories anywhere in the audit or dashboard — the render uses only `categories.performance.score` + LCP/CLS/TBT audits + `loadingExperience` field data); (2) retry transient failures up to 3x with a 2.5s×attempt backoff (5xx, bodies/messages matching lighthouseError/"something went wrong"/timeout, and AbortError/TimeoutError/network); (3) kept the 45s timeout (rarely hit now). Brand-agnostic (helps Pickl too). Proven: keyed PSI with `category=performance` returns real scores on the same pages/key (home 87, franchise 82…). `npm run check` green.

### v7.9.91 — Performance lens: custom date-range per-market traffic (any dates)
Added a "Traffic by market · custom range" section to the Performance lens, reusing the existing `market-traffic` endpoint (any startDate/endDate, all 3 segments in one call). Presets (28d/90d/6mo/12mo) + custom from/to date pickers; the per-market table (impr/clicks/CTR/avg-pos/pages) respects the branded/non-branded segment toggle and re-ranks by the active segment. Answers Shazin's "check country traffic by selecting dates". Cached by range key so toggling segment does not refetch. This absorbs the Rankings→Traffic-by-Market capability into Performance (toward retiring the Rankings triple-stack in step G). `npm run check` green.

### v7.9.90 — Reporting redesign step B/C: Performance lens — branded/non-branded toggle + visual identity
Per `NEST-REPORTING-ARCHITECTURE.md` step B (shared visual system) + first slice of C (Performance lens). Renamed the **Trends** tab → **Performance** (the CEO's month-on-month organic view). Added the reporting design system to `<head>`: **Archivo** (display) + **Hanken Grotesk** (body) Google Fonts + a namespaced `.rep-*` CSS system (framed cards, tiles, section headers, control bar — theme-aware via app tokens) — this closes the gap Shazin flagged (the live tabs wore the app's system-font skin, not the mock's Archivo/warm identity). Wired the **branded / non-branded segment toggle** (the v7.9.89 data) into the Performance lens: an All / Non-branded / Branded control now drives the MoM chart, the KPI tiles (segment impressions + clicks with MoM Δ), and the per-market table (segment impr/clicks + MoM), with a note that non-branded = SEO creating new demand. Avg-position stays all-segment (not split). Section headers + KPI numbers now render in Archivo. Additive — Report/Markets/Rankings not yet retired (that's step G, after coverage is confirmed). `npm run check` green.

### v7.9.89 — Reporting redesign step A: branded/non-branded in the monthly trend layer
Per `NEST-REPORTING-ARCHITECTURE.md` (the coherent redesign — plan around the full capability set, no more patchwork; supersedes the incremental reporting plan). Step A = the one real data gap: `monthly-trend-background` now stores a **branded / non-branded split** per month per market alongside totals. Totals stay from GSC page-only (accurate); the split comes from a second page+query pull classified by `isBrandedQuery` (same locked methodology as `market-traffic`). Non-branded impressions rising MoM = SEO creating new demand (the sharper CEO metric vs branded=awareness). Backward-compatible (`total.impressions` etc. unchanged; `.branded`/`.nonBranded` added). Backfilled from GSC's ~16 months. Next: shared control bar + visual system (step B), then the Performance lens (step C). `npm run check` green.

### v7.9.88 — P7d: CTR-gap analysis → auto Perch tasks (the "do better" lever)
Reporting refinement 7d analysis layer. `perch-sync-background` now flags a **CTR gap**: pages ranking page-one (position ≤10) with real impressions (≥200) and clicks ≥1 but CTR < 2% — a title/meta problem, not a ranking one. Auto-creates a Perch task "Low CTR — <url> (#pos, rate%)" telling the team to rewrite the title/meta. `clicks ≥1` gate keeps it from double-flagging the 0-click "optimize" finding. Live-verified: 5 real tasks (/dubai/ #8.1 @0.6% CTR, /ae/dubai/ #7.3 @0.7%, /abu-dhabi/ #9.9 @0.8%, homepage #9.4 @1.9%). 7d status: CTR-gap DONE; win/loss-by-segment already delivered (Outcomes market/type toggle, v7.9.82); time-to-rank deferred (needs ~4-6 weekly snapshots to be truthful). `npm run check` green. Reporting overhaul (7a–7d) coverage now complete — retirement of the Rankings triple-stack awaits Shazin's confirmation.

### v7.9.87 — P7c: migrated the last Report-tab pieces into Trends (coverage complete)
Brought the four remaining Report-tab surfaces into the Trends tab (below the monthly chart) so nothing is lost when the Rankings triple-stack eventually retires: **position distribution** (non-branded GSC query bands 1-3/4-10/11-20/21-50/51+), **traffic value** (AED, non-branded clicks × CPC) + non-branded clicks, **AI presence** (% keywords with AI Overview from `hasAiOverview`; AI-assistant mentions as llm-mentions `results[].mentioned` / total — v7.9.87b fixed the parse to the real shape after inspecting it live), and **rank movers** (rank-tracker top-movers / needs-attention). Fetched once per brand load into stable containers (chart toggles don't refetch). Live-verified: AED 4,530 / 906 nb-clicks / 30% AIO / 0-of-24 LLM mentions / bands 948-2535-1024-1199-357 / movers loaded. **7c coverage is now COMPLETE; retirement still deferred until Shazin confirms** (Rankings + Report tabs remain live). `npm run check` green.

### v7.9.86 — P7c increment 3: State of SEO impressions + partial-month dashing on Trends
State of SEO now surfaces **impressions** — added an "impressions · 90d" tile (was clicks-only) and friendly market names (Oman, not bonbird_oman) in the rollup + chips (impressions column was already there). Also (v7.9.85b) the Trends chart draws the in-progress current month as a **dashed, marked segment** so a partial-month dip never reads as a real drop to the CEO; KPIs/table use the last COMPLETE month. Both live-verified. 7c increments 1–3 done (data layer + Trends tab + State impressions), ADDITIVE — Rankings/Report still in place (retire only once everything's confirmed covered, per Shazin). `npm run check` green.

### v7.9.85 — P7c increment 2: Trends tab (MoM impressions, toggle-compare, per market)
The reworked Trends tab (new Analytics sub-tab, ADDED alongside Rankings/Report — nothing retired). Reads `monthlyTrend`: KPI row (impressions hero + clicks + avg position + markets, all MoM deltas vs the last COMPLETE month; the in-progress current month is labelled, not compared), an interactive **toggle-compare multi-line chart** (Total + each market, default Total + top-3 markets, click chips to add/remove — scales to any market count, colors auto-assigned from a palette by index), a 6/12-month window, and a **per-market MoM table** (impressions, MoM %, clicks, avg pos). Frontend: `loadTrends`/`renderTrends`/`trChart`/`trToggle`/`trSetWin`/`trMarketTable` in index.html, styles injected once (`#tr-styles`). Directly answers the CEO's "organic impressions up MoM, overall + per market". `npm run check` green. Next: State impressions tile (inc 3).

### v7.9.84 — P7c increment 1: monthly per-market organic trend (the CEO's MoM data layer)
Reporting refinement 7c — the data behind the reworked Trends tab (Shazin: CEO wants month-on-month **organic impressions** rising, overall AND per market). NEW `monthly-trend-background.js` builds `monthlyTrend:<brand>` = per month {total, byMarket} of impressions/clicks/avg-position, from ONE GSC page-dimension pull per month bucketed to markets by URL tokens (same attribution as the registry). FREE (GSC only, no DataForSEO). Backfills ~13 months on first run (GSC retains ~16 → real MoM history immediately), refreshes the recent 3 months weekly, caps at 18. Adding a market = config only, zero extra cost. Read via `/api/page-registry?monthly=1`. Fired weekly by `cron-weekly-background`. ADDITIVE — nothing retired (the Rankings tab stays until we're sure everything's covered, per Shazin). Next: Trends tab (toggle-compare multi-market chart) + State impressions. `npm run check` green.

### v7.9.83 — Outcomes: attention cards name + link the actual pages (click to filter)
The "Needs attention" cards showed a count but not WHICH page (Shazin: "which page is it?"). Now each card lists the actual page(s) as links (opens the page), capped at 5 + "N more", and clicking a card filters the table to those pages (noindex→status filter). noindex card points at `/om/chicken-tenders/` with the fix; drops show ▼Δ→#pos; stalled show current #pos. Live-verified.

### v7.9.82 — P7b polish: scale-ready Outcomes view (summary → attention → detail)
Rebuilt the Outcomes tab to the CEO-scorecard's polish + Option-2's data (movement Δ), and — the real ask — made it work at VOLUME (Shazin: "when we have lots of links, how are they organized?"). Structure = summary → attention → detail: window selector (30/90d/all — outcomes are about recent work), KPI tiles, a **segment rollup** (toggle by market / page type; each row a ranking/building/needs-fix stacked bar + ranking count + clicks; **click to drill into its pages**), a "needs attention" strip (noindex / ranking drops / indexed-but-0-clicks), then the full list as a **searchable + status-filterable + sortable + paginated** table with a position-Δ column. Position Δ computed from the last two weekly `pageSnapshot`s (auto-populates as weeks accrue). Styling injected once (`#oc-styles`), namespaced `.oc-*`, theme-aware via app vars. So hundreds of pages collapse to a few segment rows you drill into, never a giant table. Frontend: `loadOutcomes`/`ocRenderShell`/`ocKpis`/`ocSeg`/`ocAtt`/`ocRows` + handlers in index.html. Design explored via 2 artifact mocks first (Shazin picked the scale-ready direction). `npm run check` green. Same summary→drill pattern to be reused for State of SEO + Trends (7c).

### v7.9.81 — P7b: Outcomes view — "did our work produce results?" (the reporting gap)
Reporting refinement (NEST-REPORTING-PLAN) — the missing lens. Shazin: the tool shows state + trends but not the RESULTS OF MY WORK. New Analytics → **Outcomes** sub-tab (first tab, added ALONGSIDE the others — consolidation deferred per decision). Reads `seoEvents` (the P7a work log) joined to the registry: per brand, tiles (changes shipped · published/edited · now-ranking top-10 · clicks they drive), a "what's working" breakdown by page type (X/Y ranking), and a table of every shipped page with current position/clicks/status. Scope = ALL work (manual WP edits + Nest publishes — both captured free from sitemap `<lastmod>` in P7a). Honest: position-movement-since-ship deltas fill in as weekly snapshots accrue. Frontend: `loadOutcomes`/`renderOutcomes`/`outSelectBrand` in index.html. `npm run check` green.

### v7.9.80 — P7a: SEO event log (seoEvents) from sitemap lastmod diff + events read endpoint
Reporting refinement (NEST-REPORTING-PLAN) Phase 7a — the "work done" side of the outcome loop, which never existed durably (positionAtPublish lived on now-gone approval items). The registry build now captures each page's `<lastmod>` from the sitemap and diffs against the prior build: a NEW content page → `published` event, an advanced lastmod → `edited` event — capturing ALL work (manual WP edits AND Nest publishes) for free. First run backfills nestCreated pages (dated by lastmod) so Outcomes isn't empty. Writes append-only `seoEvents:<brand>` (dedup by type+url+at, cap 1000). Read via `/api/page-registry?events=1`. `npm run check` green.

### v7.9.79c — P6: Slack summary for auto-created Perch tasks (+ noise tightening)
Two follow-ups after live-verifying P6. (1) Noise: the "optimize" source picked up a PDF/`/wp-content/` asset and "missing-from-sitemap" false-positived on high-traffic pages (`/`, `/dubai/`) — fixed: optimize excludes non-HTML assets; missing-sitemap only fires when impressions=0 (a page with traffic is discovered regardless). Clean board = 4 high (fix-noindex + GBP ×3) + 10 medium (real optimize candidates), dedup verified (re-run creates 0). (2) Slack: auto-created tasks never pinged Slack because the sync writes tasks directly (bypassing perch.js's notify) and they're unassigned/no-due-date. Added a `perch_autosync` Slack type — ONE summary per run ("🪺 N new SEO tasks added to The Perch — 🔴 X high …"), fired only when new tasks were created (dedup keeps re-runs silent). `npm run check` green.

### v7.9.79 — P6: Perch as the action spine (findings → tracked tasks)
Refinement plan Phase 6. Perch had ZERO inbound edges (only manual CRUD + the one-time seeder) — the planned "tie everything into Perch" hook, never built. NEW `perch-sync-background.js` turns concrete SEO findings into tracked Perch tasks (writes `perchTask:<id>` + `perchIndex` directly, `source:'auto:seo'`, stable `sourceId`). SIGNAL-not-noise guardrails: only one-action findings (no raw keyword dumps), dedup by `sourceId` (never recreate any-status task), a 30/run cap, severity→priority. Sources this build: registry health flags (noindex / missing-from-sitemap / indexed-but-0-clicks top-5-by-impressions), local GBP gaps (non-UAE markets with venue pages → one task each), and ranking drops (self-activates once ≥2 weekly `pageSnapshot`s exist; 0 tasks until then). Chained off `page-registry-background` (runs on the FRESH registry) + HTTP-invocable; no own schedule (avoids the 403 trap). Curated top-keyword-target promotion deliberately DEFERRED to a fast-follow (most noise-prone; add after confirming the board stays clean). `npm run check` green.

### v7.9.78 — P4 enabler: un-trap keyword-discovery so on-demand regenerate works
Verifying v7.9.77 live surfaced a pre-existing bug: the Opportunities "regenerate" (POST `/api/keyword-opportunities` → fires `keyword-discovery-background`) was silently dead because that function had its OWN `schedule` — and Netlify 403s any HTTP call to a scheduled function (memory `netlify-function-traps`). So Opportunities only ever refreshed on the Monday cron; every manual regenerate 403'd behind the scenes (the POST returns 202 optimistically without awaiting). Fix (same pattern as scheduler-background + page-registry): removed the schedule from `keyword-discovery-background` and fire it weekly from `cron-weekly-background` (dispatcher) via `internalHeaders()`. Now HTTP-invocable → on-demand regenerate works AND the weekly refresh is preserved. This is what lets the v7.9.77 relevance-gate fix actually take effect on demand. `npm run check` green.

### v7.9.77 — P4: Opportunities relevance gate (kills "bowling near me"/"cafes near me")
Refinement plan Phase 4. The Opportunities tab surfaced obvious garbage for Bonbird (a fried-chicken brand) — "cafes near me" (vol 301k), "bowling near me", "bbq near me" — all tagged top `quick_win`. Root cause (config regression): `VERTICALS.restaurant.relevantRoots` (and `.cafe`) had re-added generic intent/location tokens (`restaurant, delivery, dine, takeaway, takeout, near me`) to the POSITIVE relevance gate, so any "…near me" passed regardless of cuisine — even though the hardcoded default `RELEVANT_ROOTS` in keyword-discovery deliberately excludes them. Fixes (config-only, both GSC + competitor candidates flow through `applyStaticFilter`, so this gates every source): (1) removed the intent/location tokens from `VERTICALS.restaurant`/`.cafe` `relevantRoots` — a keyword must carry a product/food signal; location/intent only modifies it. (2) `relevanceConfigFor` now MERGES the brand's own `offMenu` (Bonbird's peri-peri/salad/catering) into the vertical's, so brand negatives filter the opportunities engine too, not just the planner. Offline-verified: bowling/cafes/bbq/breakfast "near me" all dropped, "fried chicken near me"/"best burger dubai" kept. Regenerate (`keyword-discovery-background`) to clear stored garbage. `npm run check` green.

### v7.9.76 — P3: the "State of SEO" view (the Monday screen) + content-outcomes surfaced
Refinement plan Phase 3. New Analytics sub-tab **State of SEO** (first tab, becomes canonical; the triple-stacked Rankings tab gets retired in P7). Reads the live-page registry (`/api/page-registry`) + `/api/content-outcomes` (NEW redirect — first time closed-loop outcomes appear in the UI at all). Per brand × market: headline tiles (live pages, % in-sitemap/indexable, clicks 90d, weighted avg position, noindex flags), a by-market rollup, and the full page table (URL/type/impr/clicks/pos/status) with a market filter, plus a content-outcomes summary when present. All client-side render in `index.html` (`loadStateOfSeo`/`renderStateOfSeo`/`sosSelectBrand`/`sosSelectMarket`/`sosRebuild`), reusing the global `esc()`. Weekly trend deltas fill in once ≥2 `pageSnapshot` weeks exist. `npm run check` green (incl. inline-JS no-undef).

### v7.9.75 — P2 (additive): per-page weekly snapshot piggybacked on the registry
Refinement plan Phase 2, safe/additive part only. The registry builder now also writes `pageSnapshot:<brand>:<YYYY-Www>` on each run — a compact per-page row set (url/market/clicks/impr/position) keyed by ISO week (reruns overwrite). This is the trend series the Phase 3 "State of SEO" Monday view needs, built from the GSC page data already fetched (zero extra cost). Nothing existing changed — the risky part of Phase 2 (retiring the market-blind query-only position path in `scheduler-background`/`content-outcomes`) is DEFERRED to when Phase 3 consumes the new model, so no surface breaks mid-flight. `npm run check` green.

### v7.9.74 — page-registry fixes from live verify (noindex detection + pageType)
Verified the P1 registry live (built 205 Bonbird live pages vs ~8 the queue knew). Two bugs caught and fixed: (1) `/om/chicken-tenders/` (genuinely `noindex`) showed `unknown` because the bounded robots-fetch cap was consumed by legacy pages that already had GSC traffic — now robots is fetched ONLY for pages in real doubt (missing from sitemap AND zero impressions; a page with impressions is obviously indexed), Nest-created first, so the stray noindex is flagged. (2) pageType was off by one (Cue Cinemas, a venue, labelled `city_hub`) — now uses the configured city slugs (`citiesForMarketAsync`) to tell `city_hub` from `product` at depth-1 and treats market/city/venue depth as `venue`.

### v7.9.72 — gbp-data: clean 12h hours + real venue photos (not logos) for the app/export
The venue export delivered v7.9.71 had two flaws Shazin flagged against the Pickl app reference: hours were raw 24h per-day strings ("Sun 10:00–24:00; Mon 12:00–24:00; …") and the photo was the brand LOGO. Fixes: (1) `formatHours` now emits clean, app-style 12-hour ranges grouped by identical days — e.g. "Mon–Thu: 10 am – 12 am; Fri: 10 am – 2 am; Sat–Sun: 10 am – 12 am" — merging a period that closes at midnight with the next day's 00:00 opener so late-night shops read "… – 4 am" instead of two rows; handles Open-24-hours and Closed days. (2) the media pick now prefers a REAL venue photo — EXTERIOR → INTERIOR → FOOD_AND_DRINK → AT_WORK → ADDITIONAL → any non-logo PHOTO → COVER, falling back to PROFILE only if nothing else exists (PROFILE/LOGO/COVER are the logo/banner). Cache bumped v10→v11 to invalidate the stale logo/24h summaries.
...
### v7.9.69 — venue keyword carried 'halal' → venue copy led with a table-stakes term
scaffold-venues built the venue keyword as `${cuisine} ${city}` where cuisine=`halal fried chicken`, so the keyword was "halal fried chicken Doha" and the generator led the West Walk venue copy with "Bonbird's halal fried chicken in Doha" (commodity directive says never lead with halal, but the keyword itself pushed it in). Fix: scaffold-venues now strips the brand's commodityTerms from the cuisine before building the keyword → "fried chicken Doha". Test asserts no 'halal' in the venue keyword (14 green).

### v7.9.68 — venue scaffold made duplicates: dedup now covers pending NEST drafts, not just WP children
Publishing a hub via "Save & Publish Live" runs edit_approve THEN publish — and the venue scaffold fires on BOTH paths (handleApprove line 212 + handlePublishItem's v7.9.64 trigger). The first run's venues are pending NEST drafts, not yet WP children, so the WP `list_children` dedup couldn't see them → run #2 duplicated every venue (Doha got 2× West Walk + 2× District One). Fix: `scaffold-venues-background` now ALSO dedups against Nest venue drafts parented to the hub (any pending/approved/pushed/published status via `listApprovals`), so it's idempotent no matter how many times it fires. `scaffold-venues-test` mocks `listApprovals` + asserts a pending Nest venue draft is not regenerated (13 green). Cleaned up the live Doha dupes by hand (kept District One published + West Walk pending).
